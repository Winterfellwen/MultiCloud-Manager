# InstanceMetricsCard Component Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a new InstanceMetricsCard component that displays 4 metrics charts (CPU, Memory, Network, Disk) in a 2x2 grid.

**Architecture:** Reuse existing patterns from MetricsTab component. Create new directory `src/components/instance/` for shared instance detail components.

**Tech Stack:** React, TypeScript, Recharts, Tailwind CSS, Lucide icons

---

### Task 1: Create InstanceMetricsCard Component

**Files:**
- Create: `src/components/instance/InstanceMetricsCard.tsx`

- [ ] **Step 1: Create directory**

```bash
mkdir -p src/components/instance
```

- [ ] **Step 2: Create component file**

Create `src/components/instance/InstanceMetricsCard.tsx` with the following code:
...
```bash
git add src/components/instance/
git commit -m "feat(instance): add InstanceMetricsCard component"
```
...
1. **Spec coverage:** ✅ Component created with exact code from task description
2. **Placeholder scan:** ✅ No placeholders - all code is complete
3. **Type consistency:** ✅ Props interface matches usage, uses existing types from mock-data
